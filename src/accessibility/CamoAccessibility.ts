...
export class CamoAccessibility {
  ...
  announceBlockState(block: HTMLElement, message: string): void {
    const label = message || (block.classList.contains('camo-revealed') ? 'Revealed' : 'Hidden');
    new Notice(`CAMO: ${label}`);
  }

  makeBlockAccessible(container: HTMLElement, contentEl: HTMLElement): void {
    if (!container.hasAttribute('tabindex')) container.setAttribute('tabindex', '0');
    const contentId = contentEl.getAttribute('id') || this.ensureContentId(container, contentEl);
  ...
  applyAriaState(container: HTMLElement, contentEl: HTMLElement, revealed: boolean): void {
    container.setAttribute('aria-expanded', revealed ? 'true' : 'false');
    contentEl.setAttribute('aria-hidden', revealed ? 'false' : 'true');
  }

  private ensureContentId(container: HTMLElement, contentEl: HTMLElement): string {
