...
export class CamoSyntaxValidator {
  ...
  private validateOperators(input: string, errors: SyntaxError[]): void {
    ...
    lines.forEach((line, idx) => {
      ...
      if (/^(::|:\^:)/.test(trimmed)) {
        ...
        } else if (posTarget !== -1 && posParams !== -1 && posParams < posTarget) {
          errors.push({
        ...
        } else if (posParams !== -1 && posTrigger !== -1 && posTrigger < posParams) {
          errors.push({
            message: this.ERRORS.OPERATORS + ': -> before %',
            line: idx + 1,
            column: posTrigger + 1,
  ...
  private validateKeywords(input: string, bucket: (SyntaxError | SyntaxWarning)[]): void {
    const lines = input.split('\n');
  ...
  private validateHierarchy(input: string, errors: SyntaxError[]): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isIf = /^(::|:\^:)\s*if\{/i.test(trimmed) || /if\{[^}]+\}/i.test(trimmed);
      ...
      if (isLabelElse) {
        const hasElseSibling = stack.some(
          s => s.type === 'stmt' && lines[s.idx].trim().toLowerCase().startsWith(':: else')
        );
        if (hasElseSibling) {
          errors.push({
