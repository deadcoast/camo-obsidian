...
export class CamoPresetBuilderModal extends Modal {
  ...
  private updatePreview(el: HTMLElement) {
    ...
    (block as unknown as { setText?: (t: string) => void }).setText?.(
      "Preview content"
    );
  ...
  private async savePreset() {
    ...
    const loaded =
      (await (
        this.plugin as Plugin & { loadData: () => Promise<DataShape> }
      ).loadData()) || {};
    const customPresets: Record<string, CamoPreset> =
      loaded.customPresets || {};
    customPresets[this.preset.id] = this.preset;
    await (
      this.plugin as Plugin & { saveData: (d: DataShape) => Promise<void> }
    ).saveData({
      ...loaded,
      customPresets,
    });
